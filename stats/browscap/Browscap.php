...
/*** Se il browser non è stato riconosciuto, potrebbe essere nuovo. Qui tenta un riconoscimento dei più comuni. */
		if ($array['Browser'] == 'Default Browser')
		{
			//Nome visualizzato , stringa da cercare , stringa prima della versione
			$ua_bw = array(
						array('IE',			'/^.*Mozilla.* \(compatible; MSIE .*\).*$/',	'MSIE'),
						array('Firefox',	'/^.*Mozilla.* Gecko.* Firefox.*$/',			'Firefox/'),
						array('Opera',		'/^.*Opera.*$/',								'Opera/'),
						array('Chrome',		'/^.*Mozilla.* Gecko.* Chrome.*$/',				'Chrome/'),
						array('iPhone',		'/^.*Mozilla.* \(iPhone.*$/',					'Version/'),
						array('iPad',		'/^.*Mozilla.* \(iPad.*$/',						'Version/'),
						array('Android',	'/^.*Mozilla.* Android.*$/',					'Android'),
						array('Safari',		'/^.*Mozilla.* Gecko.* Safari.*$/',				'Version/'),
						);
			foreach ($ua_bw as $value)
			{
				if (preg_match($value[1], $user_agent))
				{
					$array['Browser'] = $value[0];

					$startpos = strpos($user_agent, $value[2]);
        			if ($startpos !== FALSE)					// è stata trovata la stringa che precede la versione
        			{
        				$res = preg_match('/[0-9]+\.[0-9]/', substr($user_agent, $startpos), $matches);
        				if ($res)								// è stato trovato il numero di versione
							$array['Version'] = $matches[0];
					}
					break;
				}
			}

			//Nome visualizzato , stringa da cercare
			$ua_os = array(
			...
			foreach ($ua_os as $value)
			{
				if (preg_match($value[1], $user_agent))
				{
					$array['Platform'] = $value[0];
					break;
				}
			}
/*** Usare solo per debug perchè gonfia molto il file di log nel caso di sito con molte visite. ***/
//			$this->_logErrors($array['Platform'].' '.$array['Browser'].' '.$matches[0]." [$user_agent]");
		}
/*** */
	...
	private function _getRemoteIniFile($url, $path)
	{
		...
		$this->_logErrors("Browscap database updated.");
		return true;
